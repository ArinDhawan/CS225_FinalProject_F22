...
Dijkstra::Dijkstra(vector<Node> dataset, unsigned start_index) {
    ...
    for (unsigned i = 0; i < dataset.size(); i++) {
        ...
        path.push_back(pair<dataset[i]._idx, -1>);  // insert all nodes, and make their 'previus' node invalid
...
void Dijkstra::solve() {
    ...
    while (edges_.size() != 0) {
        ...
        unsigned start_node_idx = get_node_idx(edges_[i]._start_node_idx);
        unsigned end_node_idx   = get_node_idx(edges_[i]._end_node_idx);

        unsigned end_dist = distances_[start_node_idx] + edges_[shortest_idx]._weight;
...
void Dijkstra::add_node_to_visited(unsigned node_index) {
    ...
    unsigned idx = get_node_idx(node_idx);

    nodes_visited_[idx] = true;    // add current node to the visited list

    for (int i = 0; i < node._edges.size(); i++) {
        edges_.push_back(node._edges[i]);
    }
